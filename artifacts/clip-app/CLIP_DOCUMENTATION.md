...
- **Индикатор обрыва анализа** — если последний символ `clip.summary` не является знаком препинания (`. ! ? » ) ] —`), под конспектом появляется предупреждение «⚠ Анализ мог быть обрезан. Попробуй уменьшить глубину или число модулей в настройках.» (фон `accentSubtle`, рамка `accentDim`, текст `textMuted` 11px).
...
- `max_tokens` в запросах — динамический, вычисляется через `getMaxTokens(depth, modules)` (от 800 до 4000, подробнее — раздел «Динамический лимит ответа»).
...
- **Блок AI-анализа** — после успешной суммаризации:
  ...
  - **индикатор обрыва** — если последний символ не знак препинания, предупреждение «⚠ Анализ мог быть обрезан…».
