...
- Данные хранятся в `Settings.aiProvider | aiApiKey | aiDepth | aiModules` (`@clip:settings`). Yandex FolderID — отдельным ключом `@clip:yandex_folder_id`. Конспект — в `Clip.summary` (`@clip:clips`). Полный текст статьи — в `Clip.linkPreview.fullText` (`@clip:clips`).
